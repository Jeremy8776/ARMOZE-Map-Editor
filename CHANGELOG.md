...
### Fixed
- **Lucide icon hydration**: routed icon rendering through a guarded helper so dynamic UI no longer passes DOM node lists as Lucide icon registries.
- **Release version progression**: moved the current app state past the installed `1.6.33` build so desktop auto-update can detect the next release.
